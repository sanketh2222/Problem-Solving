...
public class RestaurantServiceMockitoTestStub {
  ...
  @Mock
  protected RestaurantRepositoryServiceDummyImpl restaurantRepositoryServiceMock;
  ...
  public void initializeRestaurantObjects() throws IOException {
    ...
    restaurant4 = restaurants[3];//14  11 and 14 off peak
  ...
  public void  testFindNearbyWithin3km() throws IOException {
    ...
    List<Restaurant> restaurantList1 = Arrays.asList(restaurant2,restaurant4);
    List<Restaurant> restaurantList2 = Arrays.asList(restaurant1,restaurant4);;
    ...
    lenient().doReturn(restaurantList1)
        .when(restaurantRepositoryServiceMock)
        .findAllRestaurantsCloseBy(eq(20.0), eq(30.2), eq(LocalTime.of(3, 0)),//non peak
            eq(5.0));

    lenient().doReturn(restaurantList2)//11 and 14
        .when(restaurantRepositoryServiceMock)
        .findAllRestaurantsCloseBy(eq(21.0), eq(31.1), eq(LocalTime.of(7, 0)),//peak time
            eq(3.0));

    GetRestaurantsResponse allRestaurantsCloseByOffPeakHours;
    GetRestaurantsResponse allRestaurantsCloseByPeakHours;

    GetRestaurantsRequest getRestaurantsRequestPeakHours = new GetRestaurantsRequest(20.0,30.2);
    GetRestaurantsRequest getRestaurantsRequestOffPeakHours = new GetRestaurantsRequest(21.0,31.1);

    allRestaurantsCloseByOffPeakHours = restaurantService.findAllRestaurantsCloseBy(getRestaurantsRequestOffPeakHours, LocalTime.of(7, 0));
    allRestaurantsCloseByPeakHours = restaurantService.findAllRestaurantsCloseBy(getRestaurantsRequestPeakHours, LocalTime.of(3, 0));
    // TODO: CRIO_TASK_MODULE_MOCKITO
    ...
    assertEquals(2, allRestaurantsCloseByOffPeakHours.getRestaurants().size());//12 14
    assertEquals("11", allRestaurantsCloseByOffPeakHours.getRestaurants().get(0).getRestaurantId());//12
